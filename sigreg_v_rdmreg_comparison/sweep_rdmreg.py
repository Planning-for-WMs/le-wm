#!/usr/bin/env python3
"""
Sweep RDMReg checkpoints across CEM n_steps values and generate a publication plot.

Usage (from inside the le-wm conda env):
    python sweep_rdmreg.py

Produces:  rdmreg_sweep.pdf
"""
...
MODEL_PREFIX = "lewm_rdmreg_epoch"
...
def run_single(epoch: int, n_steps: int) -> tuple:
    ...
    outfile = f"rdmreg_sweep/epoch{epoch:02d}_nsteps{n_steps:02d}.txt"
...
def plot(results: list, out: str = "rdmreg_sweep.pdf") -> None:
    """Single publication-quality axes: one line per epoch, x=n_steps, y=success."""

    # organise into epoch → {n_steps: sr}
    data: dict = {}
    ...
    # plasma: dark purple (early) → yellow (late) — shows training progression
    palette = cm.get_cmap("plasma", n_epochs + 2)
    ...
    # colour-bar–style annotation showing training direction
    ax.annotate(
        "earlier → later epoch",
